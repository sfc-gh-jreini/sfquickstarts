...
an evaluation dataset. The eval table must have QUERY_TEXT (VARCHAR) and
GROUND_TRUTH (VARCHAR) columns — GROUND_TRUTH must be a plain string,
not a JSON object. Store it in SELF_IMPROVING_AGENT_DB.AGENTS and register
it as a new evaluation dataset.
```
